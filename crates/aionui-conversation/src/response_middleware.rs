...
pub struct MiddlewareResult {
    ...
    pub loaded_managed_skills: Vec<crate::skill_resolver::ManagedSkillSnapshot>,
}
...
impl MessageMiddleware {
    ...
    pub async fn process(&self, message: &str, _user_id: &str, _conversation_id: &str) -> MiddlewareResult {
        ...
        let (system_responses, loaded_managed_skills) = self.load_requested_skills(&skill_names).await;

        MiddlewareResult {
            message: cleaned,
            display_message: None,
            system_responses,
            loaded_managed_skills,
        }
    }

    async fn load_requested_skills(
        &self,
        names: &[String],
    ) -> (Vec<String>, Vec<crate::skill_resolver::ManagedSkillSnapshot>) {
        if names.is_empty() {
        ...
        let managed = loaded.iter().filter_map(|skill| skill.managed.clone()).collect();
        let responses = loaded
            .into_iter()
