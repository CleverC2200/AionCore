...
#[tokio::test]
async fn tools_list_returns_all_16_tools() {
    let env = setup().await;
    let mut stream = connect_and_init(env.server.port(), "test-token-123", "lead-1").await;
    ...
    assert!(names.contains(&"team_work_create".to_owned()));
    assert!(names.contains(&"team_work_list".to_owned()));
    assert!(names.contains(&"team_work_command".to_owned()));
